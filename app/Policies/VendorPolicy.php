<?php

namespace App\Policies;

use App\Models\User;
use App\Models\Vendor;
use Illuminate\Auth\Access\HandlesAuthorization;

class VendorPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the vendor can view any models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function viewAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the vendor can view the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function view(User $user, Vendor $model)
    {
        return true;
    }

    /**
     * Determine whether the vendor can create models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return true;
    }

    /**
     * Determine whether the vendor can update the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function update(User $user, Vendor $model)
    {
        return true;
    }

    /**
     * Determine whether the vendor can delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function delete(User $user, Vendor $model)
    {
        return true;
    }

    /**
     * Determine whether the user can delete multiple instances of the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function deleteAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the vendor can restore the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function restore(User $user, Vendor $model)
    {
        return false;
    }

    /**
     * Determine whether the vendor can permanently delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\Vendor  $model
     * @return mixed
     */
    public function forceDelete(User $user, Vendor $model)
    {
        return false;
    }
}
